...
\title{\Huge Dossier entreprise}
\author{\Large Matthieu \bsc{Balondrade} }
\date{\Large Décembre 2019}
...
professionnelle dans l'entreprise. 
	\item[Relation client : ] Les clients de \bsc{Docdoku} passent par se service pour consulter et accéder aux différents produits. Formations, prestation informatique, suivi de projet. Les différents membres de ce service sont à l'écoute
de parties prenantes externes actuelles et futures.
	\item[Technologies : ] Chez \bsc{Docdoku}, l'effectif est concentré dans le service technologique. Du fait des missions, des prestations de formations et de l'outil open source qu'elle propose, elle dispose de membres polyvalent 
se pretant aux différents besoin en matière de développement logiciel. Ce service est géré par Florent \bsc{Garin}, fondateur de \bsc{Docdoku}.
\end{description}
...
\begin{landscape}
	...
	\caption{Organigramme de \bsc{Docdoku}}
	\end{figure}
\end{landscape}

\chapter*{Missions}

\bsc{Docdoku} est une entreprise créatrice de solution digitales métier. Impliquée dans l'Open Innovation, elle développe DocDokuPLM , une plateforme Open Source de Business Data Management qui facilite la gestion des données des entreprises..
...
\begin {description}
	\item[\bsc{Airbus} : ] Dans la formation aux langages frontend et sur l'architecture logicielle web. Egalement \bsc{Docdoku} a réalisé une mission de développement logiciel utilisant sa plateforme DocdokuPLM
	\item [\bsc{Air France} : ] Afin de répondre à un projet client, Air France a consulté \bsc{Docdoku} pour la montée en compétences de ses collaborateurs dans les domaines du développement Android. Elle a également réalisé un projet mobile chez \bsc{Docdoku}
	\item [\bsc{Continental} :] Réalisation d'une application mobile multiplateformes embarquée permettant une aide à la conduite.
	\item [\bsc{SNCF} :] Réaliser l'audit de l'application TER mobile et recommander les actions qui en incombent, mener l'étude technique préalable et chiffrer le projet d'application tactile pour agents d?escales
\end{description}
...
\begin{description}
	\item [\bsc{Honeywell} : ] Pilotant les capteurs photos, l'API Camera d'Android a évolué et présente une nouvelle génération : Camera2. DocDoku a effectué la mise à jour iso-fonctionnelle sur la derniere version du SDK (Software development Kit) permettant à ses clients de se connecter à ses systèmes.

\end{description}

\section*{Quelques missions importantes confiés à \bsc{Docdoku}}

\paragraph{\bsc{Amazon}}
...
\begin{figure}
	\center
	\includegraphics[scale = 0.2]{mb1.png}
	\caption{Home page de l'application presse \bsc{Air France}}
\end{figure}

\singlespacing
\begin{figure}
	\center
	\includegraphics[scale = 0.2]{mb2.png}
	\caption{Kiosque de l'application presse \bsc{Air France}}
\end{figure}

\chapter*{Communication}

\paragraph{Communication externe}
Convaincus que pour inventer il faut s'ouvrir au monde extérieur, \bsc{Docdoku} partage quotidiennement ses expériences et compétences au travers d'ouvrages, 
de contributions Open Source, de son blog ou de nombreux événements et séminaires auxquels elle participe.
...
\caption{Évènement \bsc{Meetup} à \bsc{Docdoku}}
\end{figure}
...
\begin{description}
	\item[Google suite :] L'entreprise \bsc{Docdoku} fait bénéficier à tout ses employés un compte google + permettant de communiquer par le biais de mail, partage de document, et communauté interne.
...
Je suis employé chez \bsc{Docdoku} en tant que Développeur logiciel Full-Stack. Bien que j'appartienne au service technologique, il n'y a pas de hiérarchie précise au sein de l'entreprise.
Je suis rattaché à mon tuteur \bsc{Florent Garin} qui peux m'apporter de l'aide dans le domaine de connaissance Technique et Management ainsi qu'au Lead-Developpeur \bsc{Morgan Guimard}, plus spécialisé en développement logiciel.
...
\begin{description}
	\item[Noëline \bsc{Pagesy}:] Développeuse IOS 
	\item[Andy \bsc{Dufrenot}:] Développeur Android
...
Actuellement il m'a été confié la réalisation d'un projet auprès de notre client \bsc{Honeywell}.
...
\begin{description}
	...
	\item[Coté serveur:] Réalisation d'une API permettant les différentes opérations REST sur les ressources : Organisations, groupes, configuration, utilisateurs, filtres ...
\end{description}
...
J'ai été affecté à la réalisation du coté serveur.
...
La mission est encore en cours de développement actuellement.
